...
//		$this->load->helper(array('form'，'url'));
    ...
    public function temp_upload_image()
    {
        ...
        if (!empty($_FILES)) {
            ...
            $fileName = $partner_id."-".preg_replace('/[^\w\._]+/', '-', $fileName);
            ...
            $target_file = $target_dir . basename($return_filename);
            $uploadOk = 1;
            $imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
            // Check if image file is a actual image or fake image
            //$check = getimagesize($_FILES["file"]["tmp_name"]);
            if (move_uploaded_file($_FILES['file']['tmp_name'], $target_dir.$return_filename)) {
                die($return_filename);
            } else {
...

    {
        ...
        // Settings
		$folder = ($this->input->get('folder') != "") ? $this->input->get('folder') : 'tmp';
