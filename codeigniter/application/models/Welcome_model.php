...
public function search_reservation($keyword, $fromdate, $todate){
    ...
    WHERE COALESCE(r.res_count,0)=0
        		");
    	...
    	public function search_boat($keyword, $boat_owner, $weekdayprice, $status, $sort_field, $sort_order){

    	$where = "where (boat_name LIKE '%".$keyword."%' OR boat_owner LIKE '%".$keyword."%' OR status LIKE '%".$keyword."%')";

    	

    	if(!empty($weekdayprice) && $weekdayprice > 0){

    		switch($weekdayprice) {

	    		case '1':

	    			$where = $where." AND weekdayprice >=1 AND weekdayprice < 1999 ";

	    			break;

	    		case '2':

		    		$where = $where." AND weekdayprice >=2000 AND weekdayprice < 3999 ";

	    			break;

	    		case '3':

	    			$where = $where." AND weekdayprice >=4000 AND weekdayprice < 5999 ";

	    			break;

	    		case '4':

	    			$where = $where." AND weekdayprice >=6000 AND weekdayprice < 7999 ";
      ...
      if(!empty($status) && $status == "可用")
        $where = $where." AND status = '可用'";
      if(!empty($status) && $status == "不可用")
        $where = $where." AND status = '不可用'";
